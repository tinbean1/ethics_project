...
function SeverityBadge({ severity }) {
  if (severity === 'HIGH') return (
    <span className="badge-high flex items-center gap-1">
      <span className="w-1.5 h-1.5 bg-red-400 rounded-full"></span>HIGH RISK
    </span>
  );
  if (severity === 'MEDIUM') return (
    <span className="badge-medium flex items-center gap-1">
      <span className="w-1.5 h-1.5 bg-yellow-400 rounded-full"></span>MEDIUM
    </span>
  );
  return (
  ...
  return (
    <div className="card p-5 hover:border-[#3D3D3D] transition-colors">
      ...
      {breach.DataClasses && breach.DataClasses.length > 0 && (
        <div className="mb-3">
          <p className="text-xs text-[#718096] font-medium uppercase tracking-wide mb-1.5">Compromised data</p>
      ...
      {breach.Description && (
        <p className="text-sm text-[#A0AEC0] leading-relaxed" dangerouslySetInnerHTML={{ __html: breach.Description }} />
      )}
    </div>
  );
}
  ...
  if (isLoading) return (
    <div className="card p-8">
      <div className="animate-pulse space-y-4">
        ...
        <div className="space-y-3 mt-6">{[1,2,3].map(i => <div key={i} className="h-24 bg-[#2D2D2D] rounded-xl"></div>)}</div>
      </div>
    </div>
  );
    ...
    <div>
      ...
      <div className="card p-8">
        <div className="flex items-start justify-between gap-4 mb-6">
          ...
          {breaches.length > 0 && (
            <div className="flex-shrink-0 bg-red-950 border border-red-800 rounded-xl px-5 py-3 text-center">
              ...
              <p className="text-xs text-red-600 font-medium">{highCount > 0 ? `${highCount} high risk` : 'breaches'}</p>
            </div>
          )}
        </div>
        {breaches.length === 0 && (
          <div className="bg-green-950 border border-green-800 rounded-xl p-6">
            <p className="text-green-300 font-bold text-base mb-1">No breaches found</p>
            <p className="text-green-700 text-sm">This email does not appear in the HaveIBeenPwned database.</p>
            <p className="text-[#718096] text-xs mt-3">This does not guarantee your data has never been exposed — not all breaches are reported to HIBP.</p>
          </div>
        )}
        {breaches.length > 0 && (
        ...
        <p className="mt-6 text-xs text-[#718096] border-t border-[#2D2D2D] pt-4">
          ...
          <a href="https://haveibeenpwned.com" target="_blank" rel="noopener noreferrer" className="text-red-400 hover:underline">HaveIBeenPwned.com</a>
